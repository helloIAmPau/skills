...
| [`javascript`](javascript/SKILL.md) | Before writing or reviewing **JavaScript**, and before calling a diff's style wrong in review. No arrow functions, no space before a `function`'s parens, no `else`, no ternaries, at most two arguments, template literals for anything spanning lines, promise chains in services and `async`/`await` only in tests. There is no linter; this is the whole of the enforcement. |
