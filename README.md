...
| [`architecture`](architecture/SKILL.md) | Before adding a service or a workspace, naming either, deciding what belongs in a library, changing the ingress, or touching the Dockerfile. A service per path prefix; the data API is GraphQL and is called `graphql`; `auth` is not GraphQL and the reason matters. |
| [`javascript`](javascript/SKILL.md) | Before writing or reviewing **JavaScript**, and before calling a diff's style wrong in review. No arrow functions, no space before a `function`'s parens, no `else`, no ternaries, at most two arguments, a single-use function written where it is used, template literals for anything spanning lines, promise chains in services and `async`/`await` only in tests. There is no linter; this is the whole of the enforcement. |
