...
- **No space before the parens, ever**: `function(request, response)`, the
  same as a named declaration. Older files carry the spaced form; convert them
  when you touch them.
...
- A third positional argument is one nobody can read at the call site —
...
- **Destructure what you came for.** A callback that wants one field takes one
  field: `function({ rows })`, `function({ id })`. A query returning one row
  destructures the array — `function([ user ])` — which replaces a
  `rows.length === 0` check with `user == null` and reads better for it.
...
- **Template literals for anything spanning lines**, and for interpolation —
  `` `${ base }/auth/callback?token=${ token }` `` rather than `+`. Spaces
  inside the braces, as everywhere else.
...
- **A multi-line string is a template literal** — never concatenation, never
  an array joined on a newline. One clause per line, so the shape of the query
  is visible in the shape of the source:
  ...
  A single-line string stays in single quotes: backticks with nothing to
  interpolate and nothing to wrap are noise.
